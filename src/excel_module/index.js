...
const extractPlaceholders = (string, delimiters) => {
    ...
    while ((match = re.exec(string)) !== null) {
        // console.log('placeholder test', string);
        matches.push({
        ...
        sheetData.findall("row").forEach(row => {
            ...
            row.findall("c").forEach(cell => {
                ...
                if (cell.attrib.t === "s") {
                    ...
                    const res = extractPlaceholders(s, this.delimiters).map(p => p.placeholder.slice(this.delimiters.start.length, -this.delimiters.end.length));
                    if (res.length > 0) {
                        console.log('placeholders', res);
